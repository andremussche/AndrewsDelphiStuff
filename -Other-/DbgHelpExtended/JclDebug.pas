...
function JclCreateThreadStackTrace(Raw: Boolean; const ThreadHandle: THandle; DelayedTrace: Boolean = False): TJclStackInfoList;
...
procedure SetRemoteProcess(hProcess: THandle);
...
var
  _RemoteProces: THandle;
procedure SetRemoteProcess(hProcess: THandle);
begin
  _RemoteProces := hProcess;
end;
...
begin
  ...
  SetLength(Result, L);
  {$IFDEF MSWINDOWS}
  L := //{$IFDEF HAS_UNITSCOPE}Winapi.{$ENDIF}
       //Windows.GetModuleFileName(_RemoteProces, Module, Pointer(Result), L);
       jwaPsAPI.GetModuleFileNameEx(_RemoteProces, Module, Pointer(Result), L);
  {$ENDIF MSWINDOWS}
//  {$IFDEF UNIX}
//  {$IFDEF FPC}
//  L := 0; // FIXME
...
begin
  ...
  VirtualQueryEx(_RemoteProces, Addr, MI, SizeOf(MI));
...
const
  ...
  ImageHlpDllName = 'DbgHelpExtra.dll';                          //AM
...
begin
  ...
  begin
    ...
    try
      ...
      SymbolW^.MaxNameLen := SymbolNameLength;
      Displacement := 0;
...
begin
  ...
  if GetThreadContext(ThreadHandle, C) then
    Result := JclCreateStackList(Raw, DWORD(-1), Pointer(C.Eip), DelayedTrace, Pointer(C.Ebp),
                Pointer(GetThreadTopOfStack(ThreadHandle)));
  {$ENDIF CPU32}
  {$IFDEF CPU64}
  if GetThreadContext(ThreadHandle, C) then
    Result := JclCreateStackList(Raw, DWORD(-1), Pointer(C.Rip), DelayedTrace, Pointer(C.Rbp),
                Pointer(GetThreadTopOfStack(ThreadHandle)));
  {$ENDIF CPU64}
end;
...
begin
  ...
  begin
    ...
    if StackDataSize > 1 * 1024 * 1024 then  //bigger that 1M? probably something went wrong...
      StackDataSize := 0;
  ...
  begin
    try
      if AExceptThread then
        ...
        List := JclCreateThreadStackTrace(True, AThreadHandle, True);
...
function GetPIDOfThread(aThread:THandle): Cardinal;
type
  _CLIENT_ID = record
...
begin
  Result := 0;
  if NtQueryInformationThread(aThread, THREADBASICINFORMATION,
                              @ThreadInfo, SizeOf(ThreadInfo), @BytesRead) = 0 then
  begin
    Result := ThreadInfo.ClientId.UniqueProcess;
  end;
end;
...
begin
  ...
  try
      ...
      begin
        ...
        begin
          if SearchThreadInArray(AIncludeThreadIDs, ThreadEntry.th32ThreadID) or
             not SearchThreadInArray(AExcludeThreadIDs, ThreadEntry.th32ThreadID)
          then
    ...
    begin
      ...
      if ThreadHandle <> 0 then
        if Cardinal(ThreadIDList[I]) <> GetCurrentThreadId then
          if SuspendThread(ThreadHandle) = Cardinal(-1) then
            ThreadHandleList[I] := nil;
    end;
    try
      for I := 0 to ThreadIDList.Count - 1 do
      begin
        ...
        //some checks, threads can disappear in the mean time
        if ThreadHandle = 0 then Continue;
        if GetExitCodeThread(ThreadHandle, itemp) and
           (itemp <> STILL_ACTIVE )
        then
          Continue;
        if GetPIDOfThread(ThreadHandle) <> GetCurrentProcessId then Continue;
