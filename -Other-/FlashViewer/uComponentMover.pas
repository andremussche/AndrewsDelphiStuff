...
begin
  ...
  begin
    ...
    PositionNodes(TWinControl(Sender));
          ...
          (Sender is TGraphicControl) then
  begin
    FInReposition:=True;
//    SetCapture(TGraphicControl(Sender).Handle);
    GetCursorPos(oldPos);
    PositionNodes(TGraphicControl(Sender));
  end;
end;
...
procedure TMover.PositionNodes(AroundControl: TControl);
var
...
{      if FindPropInfo(ctrl, 'OnClick') <> nil then
      begin
...
//      newMethod.Code := self.MethodAddress('ControlOnResize');
//      newMethod.Data := Pointer(self);
//      SetMethodProp(ctrl, 'OnResize', newMethod);
    end;
  end
  else //disabled
  begin
...
//      newMethod.Code := self.MethodAddress('ControlOnResize');
//      newMethod.Data := Pointer(self);
//      SetMethodProp(ctrl, 'OnResize', oldMethod);
    end;
  end;
