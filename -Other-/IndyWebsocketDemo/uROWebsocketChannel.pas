...
type
  TCustomDataEvent = procedure(aData: TMemoryStream) of object;
...
//    fDisableNagle: boolean;
    ...
    FCustomDataEvent: TCustomDataEvent;
  ...
  public
    ...
    property OnCustomData: TCustomDataEvent read FCustomDataEvent write FCustomDataEvent;
  published
...
begin
  ...
  //FClient.SendBinary(aRequest);  does not work in browser + json message?
  ...
  while FStreamRead = nil do
    Sleep(10);
    //CheckSynchronize(10); not needed anymore
...
type
  ...
  public
    function ReadChars(aLength: Integer): AnsiString;
  end;
...
begin
  aData.Position := 0;
  sprefix := aData.ReadChars(Length(C_CUSTOMDATA));
  if sprefix = C_CUSTOMDATA then
  begin
...
function TStreamHelper.ReadChars(aLength: Integer): AnsiString;
begin
  SetLength(Result, aLength);
  Read(Result[1], Length(Result) * StringElementSize(Result));
end;
