...
RemObjects.UTIL = {
    ...
    testBrowserBinary : function testBrowserBinary() {
        var result = "";
        if (!(((typeof(XMLHttpRequest) != 'undefined') && 
		       (typeof(XMLHttpRequest.prototype.sendAsBinary) != 'undefined')
			  ) || 
			  (typeof(WebKitBlobBuilder) != 'undefined') || 
			  (typeof(BlobBuilder) != 'undefined') ||
			  (typeof(MozBlobBuilder) != 'undefined') ||
 			  /* andre.mussche@gmail.com: BlobBuilder is deprecated, should use "Blob" instead */
...
RemObjects.SDK.BinMessage.prototype.initialize = function initialize(aServiceName, aMethodName, aMessageType) {
    ...
    header.setMessageType(aMessageType || RemObjects.SDK.Enum.BinMessage.mtMessage);
    header.setClientID(this.fClientID);
    this.fRequestObject = header.asStream();
    this.parser = new BinaryParser();
    if (aMessageType != RemObjects.SDK.Enum.BinMessage.mtPoll) {
...
RemObjects.SDK.BinMessage.prototype.setResponseStream = function setResponseStream(aResponse) {
    ...
    switch (header.getMessageType()) {
        ...
        case RemObjects.SDK.Enum.BinMessage.mtException :
            ...
            throw __e;
            break;
        case RemObjects.SDK.Enum.BinMessage.mtPollResponse :
        case RemObjects.SDK.Enum.BinMessage.mtEvent :
            break;
...
RemObjects.SDK.EventReceiver.prototype.getActive = function getActive(aValue) {
    return this.fActive;
};
...
RemObjects.SDK.EventReceiver.prototype.intPollServer = function intPollServer() {
    try {
        ...
        msg.initialize(this.fServiceName, "", RemObjects.SDK.Enum.BinMessage.mtPoll);
        msg.writeInteger(10);  //maxEvents
        msg.write("", "Guid", msg.getClientID());
        msg.finalize();
        this.fChannel.dispatch(msg, function (__message) {
            var msgCount = __message.read("", "Integer");
            var msgLeft = __message.read("", "Integer");
            if (msgCount !=0) {
                __message.setResponseStream(__message.read("", "Binary"));
            };
            for (var i = 0; i < msgCount; i++) {
                var sinkName = __message.read("", "AnsiString");
                if (RemObjects.SDK.RTTI[sinkName] && RemObjects.SDK.RTTI[sinkName].prototype instanceof RemObjects.SDK.ROComplexType) {
                    ...
                    var eventName = __message.read("", "AnsiString");
                    sink.readEvent(__message, eventName);
                    ...
                    if (this.AJAX.sendAsBinary) {
                        this.AJAX.sendAsBinary(passData);
                            ...
                            else if (window.Blob) 
							{
                                ...
                                throw new Error("AjaxWrapper.post: This browser does not support BinMessage (sendAsBinary or blob builder is not defined).");
                    };
                 } else {
