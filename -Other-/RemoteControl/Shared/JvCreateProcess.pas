...
uses
  Math,
  JclStrings,
  JvJCLUtils, JvJVCLUtils, JvConsts, JvResources, PsAPI;
...
begin
  if GetModuleFileNameEx(FProcessHandle, 0, szExeFile, MAX_PATH) > 0 then
    TThread.NameThreadForDebugging(Format('%s (PID: %d, %s)',
                                                 [Self.ClassName,
                                                  jwaWinBase.GetProcessId(FProcessHandle),
                                                  szExeFile]))
  else
