...
type
  ...
  private
    type
      ...
      TAsyncCallArgGenericFunc<T> = reference to function(Arg1: T): Integer;
      ...
      TAsyncCallAnonymArgFunc<T> = class(TMultiArgProcCall<TAsyncCallArgGenericFunc<T>, T>)
      protected
        function ExecuteAsyncCall: Integer; override;
      end;
  ...
  public
    ...
    class function Invoke<T>(Func: TAsyncCallArgGenericFunc<T>; const Arg1: T): IAsyncCall; overload; static;
...
{ TAsyncCalls.TAsyncCallAnonymArgFunc<T> }

function TAsyncCalls.TAsyncCallAnonymArgFunc<T>.ExecuteAsyncCall: Integer;
begin
  Result := FProc(FArg1);
end;
...
class function TAsyncCalls.Invoke<T>(Func: TAsyncCallArgGenericFunc<T>; const Arg1: T): IAsyncCall;
begin
  { Execute the function synchron if no thread pool exists }
  if GetMaxAsyncCallThreads = 0 then
  begin
    ...
    Result := TAsyncCallAnonymArgFunc<T>.Create(Func, Arg1).ExecuteAsync;
end;
